...
- `data-context`: A string that provides additional context to the assistant. Default is `null`.
- `data-identity`: A string or JSON object payload with identity fields (`name`, `email`, `phone`, `external_id`). The widget sends it to the backend through the dedicated `identity` socket event. Default is `null`.
